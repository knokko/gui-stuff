...
abstract class PmSyntaxHighlighter(
    sourceCode: List<String>,
    private val colorTheme: PmColorTheme,
    ...
    open fun isBuiltinTypeName(candidate: String) = PmBuiltinTypes.ALL.find { it.name == candidate } != null

    private fun insertTypeName(node: TerminalNode) {
        val color = if (node.text == "Any") colorTheme.error
        else if (isBuiltinTypeName(node.text)) colorTheme.builtinTypeName
    ...
    open fun isBuiltinFunctionName(candidate: String) = PmBuiltinFunctions.MAP.containsKey(candidate) ||
            PmCompiler.extraFunctions.containsKey(candidate)
...
fun pmSyntaxTextAreaStyle(
    colorTheme: PmColorTheme,
    lineHeight: Float,
    createHighlighter: (allLines: List<String>, PmColorTheme) -> PmSyntaxHighlighter
) = TextAreaStyle(
    defaultTextStyle = { allLines -> createHighlighter(allLines, colorTheme)::getTextStyle },
    focusTextStyle = { allLines -> createHighlighter(allLines, colorTheme)::getTextStyle },
