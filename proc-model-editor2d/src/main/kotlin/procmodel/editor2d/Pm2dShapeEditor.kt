...
class Pm2dShapeEditor(
    ...
    override fun processEvent(event: Event) {
        if (event is CursorClickEvent) {
            ...
            if (event.button == 0) {
                val newVertex = Pm2dVertexValue()
                newVertex.setProperty("position", Pm2dPositionValue(clickedPosition.x, clickedPosition.y))
                points.add(newVertex)
                agent.giveFeedback(RenderFeedback())
            } else {
                val maxDistance = 0.02f
                var minDistance = maxDistance
                var closestVertex: Pm2dVertexValue? = null
        ...
        if (event is KeyPressEvent) {
            ...
            fun translate(dx: Float, dy: Float) {
                val scale = 0.1f
                for (vertex in points) {
                    val oldPosition = vertex.getProperty("position").castTo<Pm2dPositionValue>()
                    val newPosition = Pm2dPositionValue(
    ...
    private fun getSelectedVertices(): Set<Pm2dVertexValue> {
    ...
    override fun render(target: GraviksTarget, force: Boolean): RenderResult {
        ...
        fun getPositionVector(vertex: Pm2dVertexValue): Vector2f {
            val position = vertex.getProperty("position").castTo<Pm2dPositionValue>()
